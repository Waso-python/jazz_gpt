...
##
from src.services.text_analyzer.scripts.parsing import (
    read_file_txt,
...
class EmotitonAnalyzer:
    ...
    def predict_model(self, text):
        ...
        d_result["negative"] = (
            result["anger"] + result["disgust"] + result["disgust"] + result["sadness"]
        )
    ...
    def find_top_user(self, dict_stats):
        ...
        return {
            "top_user_postive": top_user_postive,
            "top_value_postive": top_value_postive,
            "top_user_negative": top_user_negative,
            "top_value_negative": top_value_negative,
        }
    ...
    def string2info(self, string):
        ...
        for key in dict_person:
            ...
            if len(key) <= 2:
                continue
            info, stats = self._analyz_one_person(
                key, dict_person[key][0], dict_person[key][1]
            )
